...
| Item | Valor |
|---|---|
| URL | <https://weblab.ivanpires.dev> (HTTP → 301 → HTTPS) |
...
| Vhost nginx | `/etc/nginx/sites-available/weblab.ivanpires.dev` (link em `sites-enabled/`) |
| Certificado | Let's Encrypt, `/etc/letsencrypt/live/weblab.ivanpires.dev/` (renovação automática pelo `certbot.timer`) |
...
Não há passo no servidor: nginx serve os arquivos estáticos direto da pasta. Conferir depois: `curl -sI https://weblab.ivanpires.dev/ | head -1` (200) e `curl -s -o /dev/null -w "%{http_code}\n" https://weblab.ivanpires.dev/nao-existe` (404).
...
server {
    ...
    ssl_certificate     /etc/letsencrypt/live/weblab.ivanpires.dev/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/weblab.ivanpires.dev/privkey.pem;
    include /etc/letsencrypt/options-ssl-nginx.conf;
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem;
}

server {
    if ($host = weblab.ivanpires.dev) { return 301 https://$host$request_uri; }
    listen 80;
    server_name weblab.ivanpires.dev;
    return 404;
}
...
2. Registro DNS `A` de `weblab.ivanpires.dev` apontando para o IP do VPS (mesmo IP de `ivanpires.dev`).
3. Vhost acima em `sites-available/`, link em `sites-enabled/`, `nginx -t && systemctl reload nginx`.
4. `certbot --nginx -d weblab.ivanpires.dev` (adicionou os blocos SSL e o redirect 80 → 443).
